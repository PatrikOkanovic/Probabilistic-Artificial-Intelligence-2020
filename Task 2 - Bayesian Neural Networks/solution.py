...
class BayesianLayer(torch.nn.Module):
    ...
    def __init__(self, input_dim, output_dim, bias=True):
        ...
        self.prior_mu = torch.zeros(output_dim, input_dim)
        self.prior_sigma = torch.ones(output_dim, input_dim)
        self.weight_mu = nn.Parameter(torch.randn(output_dim, input_dim))
        self.weight_logsigma = nn.Parameter(torch.rand(output_dim, input_dim) - 2.59)  # exp(-2.59)=0.075

        if self.use_bias:
            self.bias_mu = nn.Parameter(torch.randn(output_dim))
            self.bias_logsigma = nn.Parameter(torch.randn(output_dim))
        else:
    ...
    def forward(self, inputs):
        ...
        weights = self.weight_mu + torch.exp(self.weight_logsigma) * torch.randn(self.output_dim, self.input_dim)
        if self.use_bias:
            bias = self.bias_mu + torch.exp(self.bias_logsigma) * torch.randn(self.output_dim)
        else:
            bias = None

        return F.linear(inputs, weight=weights, bias=bias)
    ...
    def _kl_divergence(self, mu, logsigma):
        ...
        prior = Normal(
            ...
            scale=torch.ones_like(logsigma),
        )
...
class BayesNet(torch.nn.Module):
    ...
    """
    def __init__(self, input_size, num_layers, width):  # added self. to layers parameters
        super().__init__()
