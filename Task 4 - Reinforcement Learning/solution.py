...
class MLPActorCritic(nn.Module):
    ...
    def step(self, state):
        ...
        with torch.no_grad():
            pi_old, _ = self.pi(state)
            action = pi_old.sample()
            v = self.v(state)
            pi, logp_a = self.pi(state, action)
            return action.item(), v.item(), logp_a.item()
...
class Agent:
    ...
    def train(self):
        ...
        for epoch in range(epochs):
            ...
            # Calculate logp_grads
            _, logp_grads = self.ac.pi(torch.as_tensor(data['obs'], dtype=torch.float32),
                                                  torch.as_tensor(data['act'], dtype=torch.float32))

            # Do 1 policy gradient update
            pi_optimizer.zero_grad()  # reset the gradient in the policy optimizer
            loss_pi = torch.sum(-data['tdres'] * logp_grads)
            ...
            for _ in range(100):
                # Calculate v_grads
                v_grads = self.ac.v(torch.as_tensor(data['obs'], dtype=torch.float32))
                criterion = nn.MSELoss()
                v_optimizer.zero_grad()
                loss_v = criterion(v_grads, data['ret'])
                loss_v.backward()
                v_optimizer.step()
